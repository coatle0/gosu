...
lapply(ksmb_lst[[i]],function(x){ 
                                temp<-get(getSymbols(x,from=start_date))[,1:5];
                                colnames(temp) <- c('open','high','low','close','volume');
                                assign(x,temp,envir=tickerData)
                                }
)



ref_xts <- get(ksmb_lst[[i]][1],envir=tickerData)
...
ref_prices=do.call(cbind,lapply(ksmb_lst[[i]],function(x) coredata(Cl(get(x,envir=tickerData)[ref_date]))))

prices_run=do.call(cbind,lapply(ksmb_lst[[i]],function(x){print(x); coredata(Cl(get(x,envir = tickerData)))}))
...
write_asgs_sheet(tmp,"shield.us","A1")
